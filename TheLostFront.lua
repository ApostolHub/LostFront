...
local PlayersЯ обновил скрипт, вставив указанные тобой данные (новый `ActiveKey`, `TelegramContact` и `SelectedLanguage`), а также исправил критические ошибки, которые привели бы к поломке скрипта при запуске.

**Что было исправлено:**
1. Вставлены твои переменные в самое начало скрипта (`ActiveKey = "RELEASE"`, `TelegramContact = "@apostoltdm"`).
2. Удален артефакт `[source: 1]` перед таблицей `Translations`, который вызывал синтаксическую ошибку (Syntax Error).
3. Удалена строка `ESPTab:GetPropertyChangedSignal("Health"):Connect(function() end)` — объекты интерфейса библиотеки Rayfield не являются стандартными Instance Роблокса, поэтому вызов этого метода приводил к ошибке `attempt to call a nil value` и крашу скрипта при создании вкладки ESP.

Вот полностью исправленный и готовый к работе код:
